...
public class CitemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener {
    ...
    private static final String TAG = "CitemTouchHelper";
    ...
    @NonNull
    CitemTouchHelper.Callback mCallback;
    private int mActionState = 0;
    int mSelectedFlags;
    List<CitemTouchHelper.RecoverAnimation> mRecoverAnimations = new ArrayList();
    private int mSlop;
    RecyclerView mRecyclerView;
    final Runnable mScrollRunnable = new Runnable() {
        public void run() {
            if (CitemTouchHelper.this.mSelected != null && CitemTouchHelper.this.scrollIfNecessary()) {
                if (CitemTouchHelper.this.mSelected != null) {
                    CitemTouchHelper.this.moveIfNecessary(CitemTouchHelper.this.mSelected);
                }

                CitemTouchHelper.this.mRecyclerView.removeCallbacks(CitemTouchHelper.this.mScrollRunnable);
                ViewCompat.postOnAnimation(CitemTouchHelper.this.mRecyclerView, this);
            }
    ...
    private CitemTouchHelper.CitemTouchHelperGestureListener mCitemTouchHelperGestureListener;
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        public boolean onInterceptTouchEvent(@NonNull RecyclerView recyclerView, @NonNull MotionEvent event) {
            CitemTouchHelper.this.mGestureDetector.onTouchEvent(event);
            int action = event.getActionMasked();
            if (action == 0) {
                CitemTouchHelper.this.mActivePointerId = event.getPointerId(0);
                CitemTouchHelper.this.mInitialTouchX = event.getX();
                CitemTouchHelper.this.mInitialTouchY = event.getY();
                CitemTouchHelper.this.obtainVelocityTracker();
                if (CitemTouchHelper.this.mSelected == null) {
                    CitemTouchHelper.RecoverAnimation animation = CitemTouchHelper.this.findAnimation(event);
                    if (animation != null) {
                        CitemTouchHelper var10000 = CitemTouchHelper.this;
                        var10000.mInitialTouchX -= animation.mX;
                        var10000 = CitemTouchHelper.this;
                        var10000.mInitialTouchY -= animation.mY;
                        CitemTouchHelper.this.endRecoverAnimation(animation.mViewHolder, true);
                        if (CitemTouchHelper.this.mPendingCleanup.remove(animation.mViewHolder.itemView)) {
                            CitemTouchHelper.this.mCallback.clearView(CitemTouchHelper.this.mRecyclerView, animation.mViewHolder);
                        }

                        CitemTouchHelper.this.select(animation.mViewHolder, animation.mActionState);
                        CitemTouchHelper.this.updateDxDy(event, CitemTouchHelper.this.mSelectedFlags, 0);
                    }
                }
            } else if (action != 3 && action != 1) {
                if (CitemTouchHelper.this.mActivePointerId != -1) {
                    int index = event.findPointerIndex(CitemTouchHelper.this.mActivePointerId);
                    if (index >= 0) {
                        CitemTouchHelper.this.checkSelectForSwipe(action, event, index);
                    }
                }
            } else {
                CitemTouchHelper.this.mActivePointerId = -1;
                CitemTouchHelper.this.select((ViewHolder)null, 0);
            }

            if (CitemTouchHelper.this.mVelocityTracker != null) {
                CitemTouchHelper.this.mVelocityTracker.addMovement(event);
            }

            return CitemTouchHelper.this.mSelected != null;
        }
        ...
        public void onTouchEvent(@NonNull RecyclerView recyclerView, @NonNull MotionEvent event) {
            ...
            if (CitemTouchHelper.this.mActivePointerId != -1) {
                int action = event.getActionMasked();
                int activePointerIndex = event.findPointerIndex(CitemTouchHelper.this.mActivePointerId);
                if (activePointerIndex >= 0) {
                    CitemTouchHelper.this.checkSelectForSwipe(action, event, activePointerIndex);
                }

                ViewHolder viewHolder = CitemTouchHelper.this.mSelected;
                if (viewHolder != null) {
                    switch(action) {
                        case 2:
                            if (activePointerIndex >= 0) {
                                CitemTouchHelper.this.updateDxDy(event, CitemTouchHelper.this.mSelectedFlags, activePointerIndex);
                                CitemTouchHelper.this.moveIfNecessary(viewHolder);
                                CitemTouchHelper.this.mRecyclerView.removeCallbacks(CitemTouchHelper.this.mScrollRunnable);
                                CitemTouchHelper.this.mScrollRunnable.run();
                                CitemTouchHelper.this.mRecyclerView.invalidate();
                            }
                            break;
                        case 3:
                            if (CitemTouchHelper.this.mVelocityTracker != null) {
                                CitemTouchHelper.this.mVelocityTracker.clear();
                            }
                        case 1:
                            CitemTouchHelper.this.select((ViewHolder)null, 0);
                            CitemTouchHelper.this.mActivePointerId = -1;
                        ...
                        case 6:
                            ...
                            if (pointerId == CitemTouchHelper.this.mActivePointerId) {
                                int newPointerIndex = pointerIndex == 0 ? 1 : 0;
                                CitemTouchHelper.this.mActivePointerId = event.getPointerId(newPointerIndex);
                                CitemTouchHelper.this.updateDxDy(event, CitemTouchHelper.this.mSelectedFlags, pointerIndex);
                            }
                    }
    ...
    public CitemTouchHelper(@NonNull CitemTouchHelper.Callback callback) {
    ...
    private void destroyCallbacks() {
        ...
        for(int i = recoverAnimSize - 1; i >= 0; --i) {
            CitemTouchHelper.RecoverAnimation recoverAnimation = (CitemTouchHelper.RecoverAnimation)this.mRecoverAnimations.get(0);
    ...
    private void startGestureDetection() {
        this.mCitemTouchHelperGestureListener = new CitemTouchHelper.CitemTouchHelperGestureListener();
        this.mGestureDetector = new GestureDetectorCompat(this.mRecyclerView.getContext(), this.mCitemTouchHelperGestureListener);
    }

    private void stopGestureDetection() {
        if (this.mCitemTouchHelperGestureListener != null) {
            this.mCitemTouchHelperGestureListener.doNotReactToLongPress();
            this.mCitemTouchHelperGestureListener = null;
        }
        ...
        if (selected != this.mSelected || actionState != this.mActionState) {
            ...
            if (this.mSelected != null) {
                ...
                if (prevSelected.itemView.getParent() != null) {
                    ...
                    CitemTouchHelper.RecoverAnimation rv = new CitemTouchHelper.RecoverAnimation(prevSelected, animationType, prevActionState, currentTranslateX, currentTranslateY, targetTranslateX, targetTranslateY) {
                        public void onAnimationEnd(Animator animation) {
                            super.onAnimationEnd(animation);
                            if (!this.mOverridden) {
                                if (swipeDir <= 0) {
                                    CitemTouchHelper.this.mCallback.clearView(CitemTouchHelper.this.mRecyclerView, prevSelected);
                                } else {
                                    CitemTouchHelper.this.mPendingCleanup.add(prevSelected.itemView);
//                                    this.mIsPendingCleanup = true;
                                    if (swipeDir > 0) {
                                        CitemTouchHelper.this.postDispatchSwipe(this, swipeDir);
                                    }
                                }

                                if (CitemTouchHelper.this.mOverdrawChild == prevSelected.itemView) {
                                    CitemTouchHelper.this.removeChildDrawingOrderCallbackIfNecessary(prevSelected.itemView);
                                }
    ...
    void postDispatchSwipe(final CitemTouchHelper.RecoverAnimation anim, final int swipeDir) {
        this.mRecyclerView.post(new Runnable() {
            public void run() {
                if (CitemTouchHelper.this.mRecyclerView != null && CitemTouchHelper.this.mRecyclerView.isAttachedToWindow() && !anim.mOverridden && anim.mViewHolder.getAdapterPosition() != -1) {
                    ItemAnimator animator = CitemTouchHelper.this.mRecyclerView.getItemAnimator();
                    if ((animator == null || !animator.isRunning((ItemAnimatorFinishedListener)null)) && !CitemTouchHelper.this.hasRunningRecoverAnim()) {
                        CitemTouchHelper.this.mCallback.onSwiped(anim.mViewHolder, swipeDir);
                    } else {
                        CitemTouchHelper.this.mRecyclerView.post(this);
                    }
                }
    ...
    boolean hasRunningRecoverAnim() {
        ...
        for(int i = 0; i < size; ++i) {
            if (!((CitemTouchHelper.RecoverAnimation)this.mRecoverAnimations.get(i)).mEnded) {
                return true;
            }
        }
    ...
    void endRecoverAnimation(ViewHolder viewHolder, boolean override) {
        ...
        for(int i = recoverAnimSize - 1; i >= 0; --i) {
            CitemTouchHelper.RecoverAnimation anim = (CitemTouchHelper.RecoverAnimation)this.mRecoverAnimations.get(i);
    ...
    public void startDrag(@NonNull ViewHolder viewHolder) {
        if (!this.mCallback.hasDragFlag(this.mRecyclerView, viewHolder)) {
            Log.e("CitemTouchHelper", "Start drag has been called but dragging is not enabled");
        } else if (viewHolder.itemView.getParent() != this.mRecyclerView) {
            Log.e("CitemTouchHelper", "Start drag has been called with a view holder which is not a child of the RecyclerView which is controlled by this CitemTouchHelper.");
        } else {
    ...
    public void startSwipe(@NonNull ViewHolder viewHolder) {
        if (!this.mCallback.hasSwipeFlag(this.mRecyclerView, viewHolder)) {
            Log.e("CitemTouchHelper", "Start swipe has been called but swiping is not enabled");
        } else if (viewHolder.itemView.getParent() != this.mRecyclerView) {
            Log.e("CitemTouchHelper", "Start swipe has been called with a view holder which is not a child of the RecyclerView controlled by this CitemTouchHelper.");
        } else {
            this.obtainVelocityTracker();
            this.mDx = this.mDy = 0.0F;
    ...
    CitemTouchHelper.RecoverAnimation findAnimation(MotionEvent event) {
                ...
                if (Math.abs(this.mDx) > Math.abs(this.mDy)) {
                    if ((swipeDir = this.checkHorizontalSwipe(viewHolder, flags)) > 0) {
                        if ((originalFlags & swipeDir) == 0) {
                            return CitemTouchHelper.Callback.convertToRelativeDirection(swipeDir, ViewCompat.getLayoutDirection(this.mRecyclerView));
                        }
            ...
            if (this.mChildDrawingOrderCallback == null) {
                this.mChildDrawingOrderCallback = new ChildDrawingOrderCallback() {
                    public int onGetChildDrawingOrder(int childCount, int i) {
                        if (CitemTouchHelper.this.mOverdrawChild == null) {
                            return i;
                        } else {
                            int childPosition = CitemTouchHelper.this.mOverdrawChildPosition;
                            if (childPosition == -1) {
                                childPosition = CitemTouchHelper.this.mRecyclerView.indexOfChild(CitemTouchHelper.this.mOverdrawChild);
                                CitemTouchHelper.this.mOverdrawChildPosition = childPosition;
                            }
...
//        boolean mIsPendingCleanup;
    ...
    private class CitemTouchHelperGestureListener extends SimpleOnGestureListener {
        private boolean mShouldReactToLongPress = true;

        CitemTouchHelperGestureListener() {
        }
        ...
        public void onLongPress(MotionEvent e) {
            if (this.mShouldReactToLongPress) {
                View child = CitemTouchHelper.this.findChildView(e);
                if (child != null) {
                    ViewHolder vh = CitemTouchHelper.this.mRecyclerView.getChildViewHolder(child);
                    if (vh != null) {
                        if (!CitemTouchHelper.this.mCallback.hasDragFlag(CitemTouchHelper.this.mRecyclerView, vh)) {
                            return;
                        }

                        int pointerId = e.getPointerId(0);
                        if (pointerId == CitemTouchHelper.this.mActivePointerId) {
                            int index = e.findPointerIndex(CitemTouchHelper.this.mActivePointerId);
                            float x = e.getX(index);
                            float y = e.getY(index);
                            CitemTouchHelper.this.mInitialTouchX = x;
                            CitemTouchHelper.this.mInitialTouchY = y;
                            CitemTouchHelper.this.mDx = CitemTouchHelper.this.mDy = 0.0F;
                            if (CitemTouchHelper.this.mCallback.isLongPressDragEnabled()) {
                                CitemTouchHelper.this.select(vh, 2);
                            }
                        }
                    }
                }
    ...
    public abstract static class SimpleCallback extends CitemTouchHelper.Callback {
    ...
    public abstract static class Callback {
        ...
        public static ItemTouchUIUtil getDefaultUIUtil() {
            return (ItemTouchUIUtil) ItemTouchUIUtilImpl.INSTANCE;
        }
        ...
        public void onMoved(@NonNull RecyclerView recyclerView, @NonNull ViewHolder viewHolder, int fromPos, @NonNull ViewHolder target, int toPos, int x, int y) {
            ...
            if (layoutManager instanceof CitemTouchHelper.ViewDropHandler) {
                ((CitemTouchHelper.ViewDropHandler)layoutManager).prepareForDrop(viewHolder.itemView, target.itemView, x, y);
            } else {
                int minTop;
                int maxBottom;
        ...
        void onDraw(Canvas c, RecyclerView parent, ViewHolder selected, List<CitemTouchHelper.RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            int recoverAnimSize = recoverAnimationList.size();

            int count;
            for(count = 0; count < recoverAnimSize; ++count) {
                CitemTouchHelper.RecoverAnimation anim = (CitemTouchHelper.RecoverAnimation)recoverAnimationList.get(count);
                anim.update();
                int countt = c.save();
                this.onChildDraw(c, parent, anim.mViewHolder, anim.mX, anim.mY, anim.mActionState, false);
                c.restoreToCount(countt);
            }
        ...
        void onDrawOver(Canvas c, RecyclerView parent, ViewHolder selected, List<CitemTouchHelper.RecoverAnimation> recoverAnimationList, int actionState, float dX, float dY) {
            int recoverAnimSize = recoverAnimationList.size();
            ...
            for(int i = recoverAnimSize - 1; i >= 0; --i) {
                CitemTouchHelper.RecoverAnimation anim = (CitemTouchHelper.RecoverAnimation)recoverAnimationList.get(i);
                if (anim.mEnded ) {
//                    && !anim.mIsPendingCleanup
