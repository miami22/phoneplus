package com.parhamcodeappsgmail.phoneplus.SideBar;
...
public class ContactScrollerAdapter implements SectionScrollAdapter {
    ...
    public ContactScrollerAdapter(List<itema> contacts) {
    ...
    private void initWithContacts(List<itema> contacts) {
        List<itema> mContacts = contacts;
        mSections = new ArrayList<>();
        Collections.sort(mContacts, itema.COMPARATOR);
        ...
        for (int i = 0; i < mContacts.size(); i++) {
            ...
            if (i== mContacts.size()-1) lastitem=true;
